...
add_bilingual(g, SD["id_type/sappho-digital"], "Sappho-Digital-ID", "Sappho Digital ID")

g.add((SD["id_type/wikidata"], RDF.type, ECRM.E55_Type))
add_bilingual(g, SD["id_type/wikidata"], "Wikidata-ID", "Wikidata ID")

g.add((SD["id_type/dbpedia"], RDF.type, ECRM.E55_Type))
add_bilingual(g, SD["id_type/dbpedia"], "DBpedia-ID", "DBpedia ID")

g.add((SD["id_type/goodreads"], RDF.type, ECRM.E55_Type))
add_bilingual(g, SD["id_type/goodreads"], "Goodreads-Werk-ID", "Goodreads Work ID")

# Geschlechtstypen
g.add((SD["gender/female"], RDF.type, ECRM.E55_Type))
add_bilingual(g, SD["gender/female"], "weiblich", "female")
...
add_bilingual(g, SD["gender/male"], "männlich", "male")
...
add_bilingual(g, SD["gender_type/wikidata"], "Wikidata-Geschlecht", "Wikidata Gender")
...
add_bilingual(g, SD["genre_type/sappho-digital"], "Sappho-Digital-Gattung", "Sappho Digital Genre")

g.add((SD["genre/lyrik"], RDF.type, ECRM.E55_Type))
add_bilingual(g, SD["genre/lyrik"], "Lyrik", "Lyric Poetry")
...
add_bilingual(g, SD["work/sappho-work"], "Sapphos Werk", "Sappho's Work")
...
add_bilingual(g, SD["work_creation/sappho-work"],
              "Entstehung von Sapphos Werk", "Creation of Sappho's Work")
...
add_bilingual(g, manifestation_uri,
              "Andreas Bagordos Sappho-Ausgabe",
...
add_bilingual(g, manifestation_creation_uri,
              "Entstehung von Andreas Bagordos Sappho-Ausgabe",
              "Manifestation Creation of Andreas Bagordo’s Sappho edition")
...
g.add((publisher_uri, RDFS.label, Literal("Patmos")))
...
g.add((pub_place_uri, RDFS.label, Literal("Düsseldorf", lang="de")))
...
with open(INPUT_FILE, newline='', encoding="utf-8") as f:
    ...
    for row in reader:
        ...
        title_frag_uri    = SD[f"title/expression/{frag_id}"]
        ...
        add_bilingual(g, frag_uri,
                      f"Fragment {frag_num} Voigt",
                      f"Fragment {frag_num} Voigt")
        ...
        add_bilingual(g, expr_creation_uri,
                      f"Expressionserstellung von Fragment {frag_num} Voigt",
                      f"Expression Creation of Fragment {frag_num} Voigt")
        ...
        add_bilingual(g, expr_uri,
                      f"Expression von Fragment {frag_num} Voigt",
                      f"Expression of Fragment {frag_num} Voigt")
        g.add((expr_uri, LRMOO.R17i_was_created_by, expr_creation_uri))
        g.add((expr_uri, ECRM.P102_has_title, title_frag_uri))
        ...
        # Titel
        g.add((title_frag_uri, RDF.type, ECRM.E35_Title))
        g.add((title_frag_uri, RDFS.label, Literal(f"Fragment {frag_num} Voigt", lang="de")))
        g.add((title_frag_uri, RDFS.label, Literal(f"Fragment {frag_num} Voigt", lang="en")))
        g.add((title_frag_uri, ECRM.P102i_is_title_of, expr_uri))
