...
The reception testimonies listed here were compiled over several years of research in national bibliographies, newspaper and periodical databases, digital text corpora, fanfiction platforms, and scholarly secondary literature, and recorded in a spreadsheet with core bibliographic data (year of composition/publication, title, author(s), genre, place, publisher, digitized copy). Using [OpenRefine](https://openrefine.org/), entries were semi-automatically linked to Wikidata (QIDs) for persons, works, places, and publishers, and a Python script then converted the spreadsheet into the TEI/XML files below.

The RDF data adds a further, more granular layer for an exemplary subcorpus: 197 Sappho fragments (OCR-digitized) and 99 reception testimonies. Intertextual phenomena — motifs, topics, plots, rhetorical topoi, characters, and references to persons, places, and works — were manually annotated in XML through repeated close-reading passes, using a custom tagset (`motiv`, `thema`, `stoff`, `topos`, `werk`, `person`, `ort`, etc.) and checked with dedicated Python scripts and a static HTML review view. Six successive Python scripts then convert this annotated XML, together with the bibliographic lists, into RDF triples — covering author data, work/manifestation data, fragment data, feature/analysis data, and intertextual-relation data, followed by a final merge and a reasoning pass with the Java-based reasoner [HermiT](http://www.hermit-reasoner.com/) — yielding 600,000+ triples. The data model is documented as an [ontology](https://sappho-digital.com/ontology.html) built on CIDOC CRM, LRMoo, and INTRO (see below and [alignments](https://sappho-digital.com/alignments.html) to further ontologies); recurring phenomena are defined in a [SKOS vocabulary](https://sappho-digital.com/vokabular.html) of 427 concepts, developed inductively from the annotations in the LOD editor [VocBench](https://vocbench.uniroma2.it/).
